...
export default function InsuranceProfessionalsPage() {
  return (
    <>
      <Section tone="ink">
        <Eyebrow className="text-accent">For insurance professionals</Eyebrow>
        ...
        <div className="mt-8 flex flex-wrap gap-3">
          ...
          <Button href="#workflow" variant="outline" size="lg" className="border-slate-400 text-paper hover:bg-paper hover:text-navy">
            See the workflow
          </Button>
        </div>
      </Section>
        ...
        <div className="grid gap-12 md:grid-cols-12">
          ...
          <ul className="md:col-span-7 grid gap-3 sm:grid-cols-2 text-sm">
            {audience.map((role) => (
              <li
                key={role}
                className="rounded-sm border border-slate-200 bg-paper-muted px-4 py-3 text-ink"
              >
                {role}
              </li>
            ))}
      ...
      <Section tone="muted" innerClassName="" >
        ...
        <div className="mt-10 grid gap-5 md:grid-cols-4">
          {workflow.map((step) => (
            <Card key={step.n}>
              <CardBody>
                <span className="text-xs font-semibold tracking-[0.22em] text-accent-deep">
                  {step.n}
                </span>
        ...
        <div className="grid items-center gap-8 md:grid-cols-12">
          ...
          <div className="md:col-span-4 md:text-right">
            ...
            <p className="mt-3 text-xs text-slate-400">
              {site.contact.email} · {site.contact.catLine}
            </p>
          </div>
        </div>
      </Section>
