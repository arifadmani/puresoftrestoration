...
3. **UFW (host firewall) silently blocked 80/443 even after the AWS Security Group was opened.** The Ubuntu AMI ships with UFW active and only `OpenSSH ALLOW` configured. After DNS was finally pointed at the Elastic IP, Let's Encrypt's HTTP-01 / TLS-ALPN-01 challenges timed out at the TCP layer — the AWS SG was fine, but UFW's INPUT chain (policy DROP) was dropping the inbound packets before they reached Caddy. The only signal from outside was a plain TCP timeout; from inside the box `ss` showed Caddy listening on `*:80`/`*:443`. Fix on 2026-05-28 was `sudo ufw allow 80/tcp && sudo ufw allow 443/tcp`, then `systemctl restart caddy` to clear ACME's rate-limit backoff. Cert was issued within ~3 seconds of the restart via tls-alpn-01. UFW also has pre-existing world-open allowances for `3000/tcp` and `8000/tcp` (Next.js + a former dev server) — Next runs on `127.0.0.1` so those rules are no-ops at the app level, but they should be removed for hygiene.
